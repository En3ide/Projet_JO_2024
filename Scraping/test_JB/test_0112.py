def convertir_sql_mysql_vers_postgresql(script_mysql):
    # Remplacer SET SQL_MODE
    script_postgresql = script_mysql.replace('SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";', '')

    # Supprimer START TRANSACTION
    script_postgresql = script_postgresql.replace('START TRANSACTION;', '')

    # Remplacer les types de données
    script_postgresql = script_postgresql.replace('AUTO_INCREMENT', 'SERIAL')
    script_postgresql = script_postgresql.replace('enum(', 'TEXT(')

    # Remplacer ENGINE=InnoDB
    script_postgresql = script_postgresql.replace('ENGINE=InnoDB', 'WITH (OIDS=FALSE)')

    # Supprimer DEFAULT CHARSET et COLLATE
    script_postgresql = script_postgresql.replace('DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci', '')

    # Remplacer les contraintes FOREIGN KEY
    script_postgresql = script_postgresql.replace('ON DELETE SET NULL', 'ON DELETE SET NULL')
    script_postgresql = script_postgresql.replace('ON DELETE CASCADE', 'ON DELETE CASCADE')

    return script_postgresql
...
script_postgresql = convertir_sql_mysql_vers_postgresql(script_mysql)
print(script_postgresql)
